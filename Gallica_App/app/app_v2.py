import os
import gradio as gr
from gradio_utils import list_weight_files, create_summary_message, process_drop, initialize_predictors
...
with gr.Blocks(title="Gallica BNF Benchmark Visualisation Tool by EPITA", theme=gr.themes.Soft()) as demo:
    ...
    with gr.Tab("Set Evaluation"):
        with gr.Row():
            with gr.Column():
                ...
                selected_mds, lParser_weight, dt2_weight, ref_btn, selected_cls, threshold_input = setup_model_selection_interface()
                ref_btn.click(
                    fn=update_dropdowns,
                    inputs=[],
                    outputs=[lParser_weight, dt2_weight]
                )
            ...
            btn_process_images.click(process_images, 
                                    inputs=[selected_mds, dt2_weight, lParser_weight, selected_cls, threshold_input], 
                                    outputs=[info])  
    with gr.Tab("Metrics"):
    ...
    with gr.Tab("Evaluate an Image"):
        ...
        btn_process_drop.click(process_drop, 
                                inputs=[input_file, selected_mds, dt2_weight, lParser_weight, selected_cls, threshold_input], 
